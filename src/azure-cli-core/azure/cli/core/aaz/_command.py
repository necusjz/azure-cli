...
from ._arg import AAZArgumentsSchema, AAZBoolArg, \
    ...
    AAZPaginationLimitArg, AAZPaginationTokenArg
...
class AAZCommand(CLICommand):
    ...
    def build_paging(self, executor, extract_result):
        ...
        args = self.ctx.args
        limit = args.pagination_limit
        token = args.pagination_token

        return AAZPaged(executor=executor_wrapper, extract_result=extract_result, page_size=limit, next_token=token)
